...
const dbApi: IPlannerApi = {
    columns: {
        ...
        updateFields: (id, fields) => ipcRenderer.invoke('db:columns:updateFields', id, fields),
        updateUniqueProps: (id, props) => ipcRenderer.invoke('db:columns:updateUniqueProps', id, props),
        archive: (id, archivedAt) => ipcRenderer.invoke('db:columns:archive', id, archivedAt),
        delete: (id) => ipcRenderer.invoke('db:columns:delete', id),
        reorder: (order) => ipcRenderer.invoke('db:columns:reorder', order),
        move: (columnId, direction) => ipcRenderer.invoke('db:columns:move', columnId, direction),
    },
    entries: {
        getDayEntry: (columnId, dayDate) => ipcRenderer.invoke('db:entries:getDayEntry', columnId, dayDate),
        getEntriesForWeek: (weekStart) => ipcRenderer.invoke('db:entries:getEntriesForWeek', weekStart),
        getEntriesForDateRange: (startDate, endDate) =>
            ipcRenderer.invoke('db:entries:getEntriesForDateRange', startDate, endDate),
        upsertDayEntry: (input) => ipcRenderer.invoke('db:entries:upsertDayEntry', input),
        deleteEntriesForColumn: (columnId) => ipcRenderer.invoke('db:entries:deleteEntriesForColumn', columnId),
    },
    calendar: {
    ...
    settings: {
        ...
        updateColumnsOrder: (columnIds) => ipcRenderer.invoke('db:settings:updateColumnsOrder', columnIds),
    },
    system: {
