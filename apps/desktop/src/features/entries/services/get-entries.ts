...
export async function getDayEntry(columnId: string, dayDate: string): Promise<DbResult<ColumnEntry | null>> {
    try {
...
export async function getEntriesForWeek(weekStart: string): Promise<DbResult<ColumnEntry[]>> {
    try {
...
export async function getEntriesForDateRange(startDate: string, endDate: string): Promise<DbResult<ColumnEntry[]>> {
    try {
        const entries = await prisma.columnEntry.findMany({
            where: {
                dateKey: {
