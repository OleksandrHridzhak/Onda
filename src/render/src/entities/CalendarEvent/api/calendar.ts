...
import { DbResult } from 'shared/api/db';
...
> {
    try {
        const data = await calendarTable.toArray();
...
export async function getCalendarEventsByDate(
    ...
    try {
        const data = await calendarTable.where('date').equals(date).toArray();
        return { success: true, data };
    } catch (error) {
...
export async function getCalendarEventById(
    ...
    try {
        const data = await calendarTable.get(eventId);
        if (!data) {
            return {
                success: false,
...
export async function createCalendarEvent(
    ...
    try {
        const id = await calendarTable.add(eventData as CalendarEntry);
...
export async function updateCalendarEvent(
    ...
    try {
        const updatedCount = await calendarTable.update(eventId, updates);
...
export async function saveCalendarEvent(
    ...
    try {
        const id = await calendarTable.put(eventData as CalendarEntry);
...
export async function deleteCalendarEvent(
    ...
    try {
        await calendarTable.delete(eventId);
...
export async function getCalendarEventsInRange(
    ...
    try {
        const data = await calendarTable
            .where('date')
