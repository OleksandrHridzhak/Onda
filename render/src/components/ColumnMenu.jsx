...
const TitleVisibilityToggle = ({showTitle, setShowTitle, darkMode}) => (
  <div
    className="flex items-center h-12 w-12 justify-center absolute right-0 top-0">
    <button
  ...
  <div className="mb-4">
    ...
    <div className="">
      <div
        ...
        : "bg-white border-gray-200 text-gray-800"} border rounded-lg p-2 z-10`}>
        <div className="flex flex-wrap gap-2 max-h-32 ">
          {getColorOptions({darkMode}).map((color) => (<button
            key={color.name}
            onClick={() => {
            setCheckboxColor(color.name);
            toggleColorMenu();
          }}
            className={`w-6 h-6 rounded-full ${color.bg} ${color.text || (darkMode
            ? "text-gray-200"
            : "text-gray-800")} border ${checkboxColor === color.name
            ? "ring-2 ring-offset-2 ring-indigo-500"
            : "border-transparent"} hover:ring-1 hover:ring-gray-400 transition-all duration-200`}
...
const ColumnMenu = ({
  ...
  const column = useSelector(selectColumnById(id));
  ...
  const handleAddOption = () => {
    ...
    if (state.newOption.trim() && !state.options.includes(state.newOption.trim()) && !state.doneTags.includes(state.newOption.trim())) {
      onChangeOptions(column.ColumnId, [
        ...state.options,
        state
          .newOption
          .trim()
  ...
  const handleRemoveOption = (option) => {
    ...
    const isInOptions = state
      .options
      .includes(option);
    const isInDoneTags = state
      .doneTags
      .includes(option);
    if (isInOptions) {
  ...
  const handleColorChange = (option, color) => {
    ...
    onChangeOptions(column.ColumnId, state.options, {
      ...state.optionColors,
      [option]: color
    }, state.doneTags);
  };
  ...
  const handleSave = async() => {
    ...
    try {
      ...
      const optionsChanged = JSON.stringify(state.options) !== JSON.stringify(column.Options) || JSON.stringify(state.optionColors) !== JSON.stringify(column.TagColors) || JSON.stringify(state.doneTags) !== JSON.stringify(column.DoneTags);
  ...
  return (
    <div
      className={`fixed z-50 cursor-default inset-0 flex items-center justify-center bg-black bg-opacity-50 ${darkMode
      ? "text-gray-100"
      : "text-gray-800"}`}>
      <div
        className={`w-full max-w-md rounded-2xl ${darkMode
        ? "bg-gray-900 border-gray-800"
        : "bg-white border-gray-200"} border shadow-md p-4`}
        ref={menuRef}>
        <div className="flex justify-between items-center mb-4">
          <h2
            className={`text-lg z-55 font-semibold ${darkMode
            ? "text-gray-100"
            : "text-gray-800"}`}>
            Column Settings
          </h2>
        ...
        <div className="">
          <div className="flex gap-2 w-full">
            ...
            <div className="w-full flex relative">
              <input
                ...
                className={`w-full h-[50px] px-4 py-2 border ${darkMode
                ? "border-gray-700 bg-gray-900 text-gray-200"
                : "border-gray-300 bg-white text-gray-900"} rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm transition-all duration-200`}
          ...
          <div className="">
            <textarea
              ...
              ? "border-gray-700 bg-gray-900 text-gray-200"
              : "border-gray-300 bg-white text-gray-900"} rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm transition-all duration-200 resize-none`}
              rows="3"/>
          </div>
          <div className="mb-4">
            <label
            ...
            <div className="flex space-x-2">
              <div className="w-full">
                <input
                  ...
                  min="0"
                  max="1000"
                  className={`w-full px-3 py-2.5 flex items-center text-sm border ${darkMode
                  ? "bg-transparent border-gray-700 text-gray-200 hover:text-white"
                  : "bg-transparent border-gray-300 text-gray-900"} rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-blue-600`}
          ...
          {["multi-select", "todo", "multicheckbox", "tasktable"].includes(column
            ?.Type) && (<OptionsList
          ...
          {column
            ?.Type === "checkbox" && (<CheckboxColorPicker
