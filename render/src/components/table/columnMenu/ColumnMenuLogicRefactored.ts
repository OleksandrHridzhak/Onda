...
export const useColumnMenuLogic = (
  ...
  const updateColumnInstance = useCallback(async (
    columnId: string,
  ...
  const handleRename = useCallback(async (columnId: string, newName: string): Promise<UpdateResult> => {
  ...
  // Зміна іконки
  const handleChangeIcon = useCallback((columnId: string, newIcon: string): Promise<UpdateResult> => {
  ...
  // Зміна опису
  const handleChangeDescription = useCallback(async (columnId: string, newDescription: string): Promise<UpdateResult> => {
    return await updateColumnInstance(columnId, (column) => {
  ...
  const handleToggleTitleVisibility = useCallback((columnId: string, showTitle: boolean): Promise<UpdateResult> => {
    return updateColumnInstance(columnId, (column) => {
  ...
  const handleChangeCheckboxColor = useCallback((columnId: string, color: string): Promise<UpdateResult> => {
    return updateColumnInstance(columnId, (column) => {
