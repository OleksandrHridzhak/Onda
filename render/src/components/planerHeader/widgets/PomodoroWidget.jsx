...
const PomodoroWidget = () => {
  ...
  return (
    <div
      className={`w-[150px] h-[50px] ml-2 justify-center px-2 rounded-xl flex flex-col items-center space-y-2 border bg-table-body-background border-ui-border text-text-secondary`}
    >
      {isEnded ? (
        <div className="flex flex-row items-center space-x-2">
          <div className="font-medium">{initialMinutes} ended!</div>
          <button
            className={`h-8 w-8 flex items-center justify-center rounded-full hover:bg-ui-hover text-button-danger-background`}
            onClick={handleClose}
          >
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="flex flex-row items-center justify-between w-full">
          <div className={`text-lg pl-1 font-poppins text-text-secondary`}>
            {formatTime(time)}
          </div>
          <div className="flex space-x-2">
            <button
              className={`h-8 w-8 flex items-center justify-center rounded-full hover:bg-ui-hover text-text-secondary`}
        ...
        <div className="flex items-center space-x-1">
          ...
          {[30, 25, 5].map((m) => (
            <button
              key={m}
              className={`px-2 py-1 rounded-md cursor-pointer text-text-secondary hover:bg-ui-hover`}
