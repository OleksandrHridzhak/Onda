...
import { PushRequestBody, PullRequestBody, SyncDocument } from '../types/index.js';

/**
 * Get sync data - pull from server
 */
export async function getData(req: AuthRequest, res: Response): Promise<void> {
  try {
    const { secretKey } = req;
    const collection = getCollection();
    const doc = await collection.findOne({ secretKey: secretKey! });
 ...
 */
export async function pushData(req: AuthRequest, res: Response): Promise<void> {
  try {
    const { secretKey } = req;
    const { data: clientData, clientVersion } = req.body as PushRequestBody;
    ...
    const collection = getCollection();
    const serverData = await collection.findOne({ secretKey: secretKey! });
    ...
    if (!serverData) {
      ...
      console.log(`✅ Push completed - Initial sync - Version: 1`);
      res.json({
        success: true,
        version: 1,
 ...
 */
export async function pullData(req: AuthRequest, res: Response): Promise<void> {
  try {
    const { secretKey } = req;
    const { clientVersion, clientLastSync } = req.body as PullRequestBody; // clientLastSync for backward compatibility
    ...
    const hasConflict = clientVersion !== undefined && clientVersion < serverData.version;
 ...
 */
export async function deleteData(req: AuthRequest, res: Response): Promise<void> {
  try {
    const { secretKey } = req;
    const collection = getCollection();
    await collection.deleteOne({ secretKey: secretKey! });
